...
row per coordinated target. Default Plugins and Updates screen notices are
capability-scoped, filterable through
`ran_wp_github_release_updater_notice`, sanitized after filtering, and quiet
for transient network failures.

When `WP_DEBUG_LOG` is enabled, the package emits concise stable events without
credentials, URLs, response bodies, archive contents, or filesystem paths.
...
APIs; classes under `src/Artifact` are internal and should not be constructed or
introspected by consumers.

- [Managed preflight and prospective release API](docs/managed-preflight-api.md)
  covers installed-package checks, first-install inspection and acquisition,
  artifact custody, coordination leases, cache invalidation, and request
  budgets.
- [Release assurance extension](docs/release-assurance-extension.md) defines the
  single optional, request-local, rejection-only assurance checker. It can add
  a rejection after the built-in checks, but it cannot waive a failure or
  install a package.
...
notes. After the exact candidate passes the repository's quality proof, the
publisher creates and reads back the tag and immutable GitHub source release.
This repository does not build or upload a consuming plugin's or theme's ZIP.
